...
class SolicitudPrestamosServiceImplTest {
    ...
    @Mock private TipoPlazoMapper tipoPlazoMapper;
    ...
    private TipoPlazoResponse tipoPlazoResponse;
    ...
    void setUp() {
        ...
        tipoPlazoResponse = new TipoPlazoResponse(1, 12, new BigDecimal("0.05"));
    ...
    @Test
    void cuandoGetAllTipoPlazo_deberiaDevolverPaginaDeTipoPlazoResponse() {
        
        Page<TipoPlazo> tiposPlazoPage = new PageImpl<>(Collections.singletonList(tipoPlazo));
        when(tipoPlazoRepository.findAll(pageable)).thenReturn(tiposPlazoPage);
        when(tipoPlazoMapper.toDto(any(TipoPlazo.class))).thenReturn(tipoPlazoResponse);

        
        Page<TipoPlazoResponse> resultados = solicitudPrestamosService.getAllTipoPlazo(pageable);

        
        assertNotNull(resultados);
        assertFalse(resultados.isEmpty());
        assertEquals(1, resultados.getTotalElements());
        assertEquals(tipoPlazoResponse.getPlazoID(), resultados.getContent().get(0).getPlazoID());
        verify(tipoPlazoRepository, times(1)).findAll(pageable);
        verify(tipoPlazoMapper, times(1)).toDto(any(TipoPlazo.class));
    }

    @Test
    void cuandoGetTipoPlazoById_conIdExistente_deberiaDevolverTipoPlazoResponse() {
        
        Integer plazoId = 1;
        when(tipoPlazoRepository.findById(plazoId)).thenReturn(Optional.of(tipoPlazo));
        when(tipoPlazoMapper.toDto(any(TipoPlazo.class))).thenReturn(tipoPlazoResponse);


        TipoPlazoResponse resultado = solicitudPrestamosService.getTipoPlazoById(plazoId);

        
        assertNotNull(resultado);
        assertEquals(tipoPlazoResponse.getPlazoID(), resultado.getPlazoID());
        verify(tipoPlazoRepository, times(1)).findById(plazoId);
        verify(tipoPlazoMapper, times(1)).toDto(any(TipoPlazo.class));
    }

    @Test
    void cuandoGetTipoPlazoById_conIdNoExistente_deberiaLanzarExcepcion() {
        
        Integer plazoId = 99;
        when(tipoPlazoRepository.findById(plazoId)).thenReturn(Optional.empty());

        
        assertThrows(SolicitudNotFoundException.class, () -> solicitudPrestamosService.getTipoPlazoById(plazoId));
        verify(tipoPlazoRepository, times(1)).findById(plazoId);
        verify(tipoPlazoMapper, never()).toDto(any(TipoPlazo.class));
    }
}
